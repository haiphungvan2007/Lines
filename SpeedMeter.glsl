...
#define SMALLEST_RADIUS 85.0
#define SMALLEST_BORDER_WIDTH 4.0
#define SUB_BORDER_WIDTH 2.0
#define SUB_RADIUS_DISTANCE 8.0


#define CIRCLE_OVERLAY_COLORS vec4(0.0, 0.6, 0.8, 1.0)

#define CIRCLE_COLOR0 vec4(0.1803921568627451, 0.1803921568627451, 0.1803921568627451, 1.0)
#define CIRCLE_BORDER_WIDTH0 4.0
#define CIRCLE_RADIUS0 205.0

//0x00253F;
#define BEGIN_GRADIENT1 vec4(0.06274509803921569, 0.16470588235294117, 0.24705882352941178, 1.0)
//0x0099FF
#define END_GRADIENT1 vec4(0.06666666666666667, 0.4235294117647059, 0.5803921568627451, 1.0)


#define CIRCLE_COLOR1 vec4(0.07450980392156863, 0.07450980392156863, 0.07450980392156863, 1.0)
#define CIRCLE_WIDTH1 10.0
#define CIRCLE_BORDER1 2.0
#define CIRCLE_RADIUS1 195.0
...
vec4 drawSmothCircle(float borderWidth, vec4 borderColor, float radius, vec2 centerPoint) {
    ...
    */
    ...
    // vec2 currentPoint = gl_FragCoord.xy;
    // vec2 tempDis = currentPoint - centerPoint;
    // float r = tempDis.x * tempDis.x + tempDis.y * tempDis.y;
    // vec4 retVal = BACKGROUND_COLOR;
    // if ((r > ((radius - borderWidth)*(radius - borderWidth))) && (r <  (radius*radius))) {
    //     retVal = borderColor;
    // }
    // return retVal;
...
vec3 paintCircle (vec2 uv, vec2 center, float rad, float width) {
    ...
    len += variation(diff, vec2(0.0, 1.0), 40.0, 30.0);
    len -= variation(diff, vec2(1.0, 0.0), 40.0, 30.0);
...
void main() {
    ...
    float currentPowerInPercent = min(displayPower / duration * u_time, displayPower);
    ...
    float pointPowerPercent = pointPowerAngle / maxAngle;
    float currentPowerInAngle = currentPowerInPercent * maxAngle;
    if (pointPowerAngle <= (maxAngle)) {
    ...
    for (float i = 0.0; i < NUMBER_OF_CIRCLE; i += 1.0) {
        ...
        if (i > 0.0) {
            ...
            if ((pointPowerAngle > currentPowerInAngle) &&
                (pointPowerAngle < (currentPowerInAngle + 0.7)) &&
    ...
    if ((maxAngle - currentPowerInAngle) > 4.0) {
        if (pointPowerAngle <= currentPowerInAngle) {
            vec4 tempColor = drawSmothCircle(CIRCLE_BORDER_WIDTH0, CIRCLE_OVERLAY_COLORS, CIRCLE_RADIUS0, centerPoint);
            if (!isBackgroundColor(tempColor)) {
                retColor = tempColor;
            }
        ...
        if (pointPowerAngle <= (maxAngle + 4.0)) {
            vec4 tempColor = drawSmothCircle(CIRCLE_BORDER_WIDTH0, CIRCLE_OVERLAY_COLORS, CIRCLE_RADIUS0, centerPoint);
            if (!isBackgroundColor(tempColor)) {
                retColor = tempColor;
            }
        ...
        tempColor = drawLineForCircle(225.0, CIRCLE_RADIUS0, CIRCLE_WIDTH1*2.0 / 4.0 * (pointPowerAngle - maxAngle), centerPoint, -5.0, END_GRADIENT1);
        if (!isBackgroundColor(tempColor)) {
            retColor = tempColor;
        }
    }
    ...
    tempColor = vec4(paintCircle(gl_FragCoord.xy/u_resolution.xy, vec2(0.5), (SMALLEST_RADIUS) / u_resolution.x, 0.002), 1.0);
    if (!isBackgroundColor(tempColor)) {
